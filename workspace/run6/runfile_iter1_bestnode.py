...
def evaluate_topk(recs, test_df, users, k=10):
    ...
    rec_groups = {u: g.sort_values('rank')['item'].tolist()[:k] for u, g in recs.groupby('user')}
    ndcgs, precs = [], []
    for u in users:
        gt = truth.get(u, set())
    ...
    return float(np.mean(ndcgs)), float(np.mean(precs)), truth
...
for seed in seeds:
    ...
    eval_users = np.sort(test['user'].unique())
    train_items = train.groupby('user')['item'].apply(set).to_dict()

    algo = ImplicitMF(features=50, iterations=20, reg=0.1, weight=40)
    algo.fit(train)

    def candidate_selector(user, item_set=all_item_set, seen=train_items):
        return list(item_set - seen.get(user, set()))

    recs = batch.recommend(algo, eval_users, 10, candidates=candidate_selector)
    if len(recs):
        recs = recs.sort_values(['user', 'rank']).reset_index(drop=True)

    val_loss = float('nan')
    ndcg10, p10, truth = evaluate_topk(recs, test, eval_users, k=10)
    print(f'Seed {seed}: validation_loss = {val_loss:.4f}')
    print(f'Seed {seed}: nDCG@10 = {ndcg10:.4f}, Precision@10 = {p10:.4f}')

    rows.append({'seed': seed, 'nDCG@10': ndcg10, 'Precision@10': p10})
    experiment_data['ml100k']['metrics']['val'].append({'seed': seed, 'nDCG@10': ndcg10, 'Precision@10': p10})
